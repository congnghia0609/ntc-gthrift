...
# Get a certificate signing request from the CA (client.csr)
...
# Convert the client certificate to .pem (client.pem)
openssl pkcs8 -topk8 -nocrypt -passin pass:123456 -in client.key -out client.pem
# Convert the server certificate to .pem (server.pem) - usable by Thrift
